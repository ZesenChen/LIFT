...
%  [2] R. E. Schapire, Y. Singer. BoosTexter: A boosting based system for text categorization. Machine Learning, 39(2/3): 135-168, 2000.
    ...
    for i=1:num_class
        ...
        n_idx=setdiff([1:num_train],p_idx);%返回1~num_train中不在p_idx中的数字
        ...
        if(k1==0)
            ...
            if(size(p_data,1)==1)%如果p_data只有一行
                POS_C=p_data;
            else
    ...
    for i=1:num_class        
        ...
        centers=[P_Centers{i,1};N_Centers{i,1}];%行==聚类中心数;列==特征向量维度
        ...
        if(num_center>=5000)
            ...
            num_block=ceil(num_train/blocksize);%num_block = 1
            for j=1:num_block-1
                low=(j-1)*blocksize+1;
                high=j*blocksize;
            ...
            low=(num_block-1)*blocksize+1;%low = 1
            high=num_train;%high = 400
            
            tmp_mat=[centers;train_data(low:high,:)];%聚类中心向量和训练样本的特征向量上下拼成一个矩阵
            Y=pdist(tmp_mat);
            Z=squareform(Y);%生成tmp_mat的距离方阵
    ...
    save 'model.mat' Models P_Centers N_Centers;
